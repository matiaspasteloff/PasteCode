...
| **El extension host consume el margen que quedaba de RNF-04**           | **Alto** | **Alta**     | **Nuevo en la Fase 4.** El spike S2 lo midió en **64,5MB**, y la corrida completa quedó en 396,78MB contra un techo de 400. El presupuesto no se movió a propósito. El fork perezoso no alcanza —la instalación por omisión trae una extensión con código—, así que antes de la Fase 5 hay que elegir entre partir RNF-04 como se partió por `tsserver` o bajar de verdad lo que ocupa la app |
